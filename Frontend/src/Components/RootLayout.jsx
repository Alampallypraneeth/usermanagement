...
function RootLayout() {
  return (
    <div className="relative min-h-screen bg-[#050811] text-slate-200 flex flex-col overflow-x-hidden">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] rounded-full bg-slate-700/5 blur-[130px] pointer-events-none animate-blob-1 z-0"></div>
      <div className="absolute top-[40%] right-[-10%] w-[45vw] h-[45vw] rounded-full bg-slate-800/5 blur-[120px] pointer-events-none animate-blob-2 z-0"></div>
      <div className="absolute bottom-[-10%] left-[20%] w-[40vw] h-[40vw] rounded-full bg-slate-900/5 blur-[100px] pointer-events-none animate-blob-3 z-0"></div>
      ...
      <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-[#080d1a]/75 backdrop-blur-md px-6 py-4 md:px-16 flex justify-between items-center transition-all duration-300">
        <NavLink to="/" className="flex items-center gap-3 font-extrabold text-2xl tracking-tight text-white group">
          <div className="p-2.5 bg-slate-805 border border-slate-700 rounded-xl text-slate-300 group-hover:bg-slate-750 group-hover:border-slate-600 transition-all duration-300">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-2 py-2 px-4 rounded-xl text-sm font-semibold transition-all duration-200 ${isActive
                ? "text-white bg-slate-800 border border-slate-700 shadow-sm shadow-white/5"
      ...
      <footer className="w-full py-6 text-center border-t border-white/5 bg-[#050811]/90 text-xs text-slate-500 z-10 flex flex-col sm:flex-row justify-between items-center gap-3 px-6 md:px-16">
        ...
        <div>
          <span>© {new Date().getFullYear()} User Management. All rights reserved.</span>
        </div>
      </footer>
    </div>
  );
