  ...
  return (
    <div className="glass-panel glass-panel-hover p-6 rounded-3xl flex flex-col justify-between space-y-6 border-white/5 relative overflow-hidden transition-all duration-300">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="w-12 h-12 rounded-2xl bg-slate-805 border border-slate-700 text-slate-200 font-extrabold flex justify-center items-center shadow-md text-sm shrink-0">
          {initials}
        </div>

        <div className="space-y-1">
          <h3 className="font-extrabold text-white text-lg leading-tight tracking-tight hover:text-slate-300 transition-colors duration-200">
            {user.name}
          </h3>
      ...
      <div className="flex items-center justify-end gap-2.5 pt-1">
        <button
          onClick={() => onEdit(user)}
          className="flex items-center gap-1.5 px-3.5 py-2.5 text-xs font-bold bg-slate-805 border border-slate-700 text-slate-300 hover:bg-slate-700 hover:text-white rounded-xl transition-all duration-200 hover:scale-[1.03]"
